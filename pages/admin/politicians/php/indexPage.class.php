...
class indexPage extends indexPageBase {
	...
	public function processGet($get) {
		...
		if (!isset($get['start']) || !ctype_digit($get['start']))
			$get['start'] = 0;
		$join = '';
		$where = 'WHERE t.def_party IS NULL '; //exclude special 'Onbekend' politicians
		//$where = '';
		if (!Dispatcher::inst()->user->isSuperAdmin()) {
			...
			$where .= ' AND (region = '.$_SESSION['role']->getRecord()->id.' OR region_created = '.$_SESSION['role']->getRecord()->id.')';
		}
