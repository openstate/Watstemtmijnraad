...
class Xml {
	...
	public function getRoot($name, $xsdName = null) {
		...
		return '<'.(strlen($this->namespace) ? $this->namespace.':' : '').
			$name.' xmlns'.(strlen($this->namespace) ? ':'.$this->namespace : '').'="http://www.watstemtmijnraad.nl" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://www.watstemtmijnraad.nl http'.(isset($_SERVER['HTTPS'])?'s':'').'://'.$_SERVER['SERVER_NAME'].'/xml/'.$xsdName.'.xsd">';
	}
