...
class Import {
	...
	public function __construct($db) {
		$log = JLogger::getLogger('utils.import.import');
		...
		$this->schema = new ModelSchema($db); //fetch all the data
	 ...
	 *
	 * The mode $raadsstuk_merge = true results in merge of raadsstukken.
	 * Existing raadsstuk searched first by all fields
	 * 
	 ...
	 * @param boolean $raadsstuk_merge if true, raadsstuk should be merged, otherwise simply added
	 * @return void
	 */
	public function process($raadsstuk_merge = true, &$ids = null) {
		$log = JLogger::getLogger('utils.import.import');
		...
		try {
			...
			if($this->reader->nodeType == XMLReader::ELEMENT) {
				$log->enter("Starting import of raadsstukken.");
				//read raadsstukken data
				$i = $this->importRaadsstukken($raadsstuk_merge, $ids);
				$log->leave("Successfully imported all {$i} raadsstukken.");
			} else{
				$log->info("Raadsstukken block is not found.");
			}
	 ...
	 */
	private function importRaadsstukken($raadsstuk_merge, &$ids = null) {
		$log = JLogger::getLogger('utils.import.import');
		...
		while ($this->reader->read()) {
			if($this->reader->nodeType == XMLReader::ELEMENT && $this->reader->name == 'raadsstuk') {
				...
				$log->debug("Processing raadsstuk: {$xml['title']}");
				...
				$localid = (string)$xml['local_id'] == ''? null: (string)$xml['local_id'];
				...
				if(!$xml->submitter) throw new RuntimeException("Submitter is required for raadsstuk '{$xml['title']}'!");
				else {
				...
				$vts = $xml->votes;
				if($xml->votes) {
					$vtype = (string)$vts['type'];
					$vresult = (string)$vts['result'];
				}
				$log->debug("Adding votes by: {$vtype}");
				$voteblock = $rs->defineVoteBlock($vtype, $vresult);

				if($vts) {
					foreach ($vts->vote as $vote) {
						if(isset($vote['politician']) && $vote['politician'] != '') $voteblock->votePolitician((string)$vote['vote'], (string)$vote['politician'], (string)$vote['party']);
				...
				if($raadsstuk_merge) {
					$log->debug("Merging raadsstuk: {$rs->title}");
					$rad_id = $rs->merge();
					if($ids !== null && $xml['refid']) $ids[(string)$xml['refid']] = $rad_id;
				} else {
					$log->debug("Creating new raadsstuk: {$rs->title}");
					$rad_id = $rs->create();
					if($ids !== null && $xml['refid']) $ids[(string)$xml['refid']] = $rad_id;
				}
				...
				/*} catch (NotFoundException $e) {
					$log->debug("Skipping parent raadsstuk references while debugging: ".$e->getMessage());
				} catch (Exception $e) {
