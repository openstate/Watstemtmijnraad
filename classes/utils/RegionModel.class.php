...
class RegionModel extends TreeNodeModel {
	...
	public function resolve($schema, $db, $id, $parent) {
		...
		if($id === null) { //create self
			if(defined('DRY_RUN')) throw new RuntimeException("Before inserting new region: ".$this->name);

			if(defined('FULL_ACCESS') && !FULL_ACCESS)
				throw new RuntimeException('Not allowed to insert new region: '.$this->name);
		...
		if($root) return $this->schema->addRegion($region, $path);
			...
			if(empty($els)) {
				if(!isset($this->children[$region->getKey()])) return $this->schema->resolveRegion($region, $this);
					...
					return $this->children[$region->getKey()];
				}
	...
	public static function stem($key) {
		if(defined('DISABLE_STEM_REGION')) return ModelSchema::plainNormalize($key);
		return parent::stem($key);
	}
	...
	public function toXmlWrite($xw, $options) {
		...
		//check for flat
		if(isset($options['regions.flat']) && $options['regions.flat']) {
			$xw->writeAttribute('path', $this->parent? $this->parent->getPath(): '/');
			$xw->endElement(); // </region>
			
			foreach ($this->children as $chld) $chld->toXmlWrite($xw, $options);
		} else {
			if($this->getLevel() == 1) $xw->writeAttribute('path', '/');

			if(sizeof($this->children) > 0) {
				$xw->startElement('regions'); // <regions>
				
				foreach ($this->children as $chld) $chld->toXmlWrite($xw, $options);
				
				$xw->endElement(); // </regions>
			}
	...
	public function touch() {
		...
		if($this->parent) $this->parent->touch();
	}
}
