...
class SiteModel {
	...
	public function resolve(SiteModelSchema $schema, $db, $id) {
		...
		if($id === null) {
			if(defined('DRY_RUN')) throw new RuntimeException("Before inserting new site: ".$this->name);

			if(defined('FULL_ACCESS') && !FULL_ACCESS)
				throw new RuntimeException('Not allowed to insert new site: '.$this->name);
	...
	public static function stem($key) {
		if(defined('DISABLE_STEM_SITE')) return ModelSchema::plainNormalize($key);
