...
class PartyModelSchema {
	...
	public function __construct($db, ModelSchema $global_schema) {
		...
		foreach ($ret as $row) {
			...
			$party->resolve($this, $this->db, $row['id']);
	...
	public function addParty(PartyModel $party) {
		...
		$party->resolve($this, $this->db, null);
	...
	public function lookup($id) {
		if(!isset($this->id_index[$id])) {
			...
			$par->resolve($this, $this->db, $row['id']);
	...
	public function toXmlWrite($xw, $options = null) {
		$xw->startElement('parties'); // <parties>
		...
		while(sizeof($comb) > 0) {
			$par = array_pop($comb);
			if(!isset($done[$par->getKey()])) $this->depToXmlWrite($xw, $options, $done, $par);
		}
		
		$xw->endElement(); // </parties>
	}
	
	protected function depToXmlWrite($xw, $options, &$done, $par) {
		foreach ($par->listCombinationParties() as $prnt) {
			if(!isset($done[$prnt->getKey()])) $this->depToXmlWrite($xw, $options, $done, $prnt);
		}
		$par->toXmlWrite($xw, $options);
		$done[$par->getKey()] = true;
	}
	...
	public function update(SimpleXMLElement $node) {
		...
		foreach ($node->party as $par) {
			$region = (string)$par['region'];
			$region = $this->global_schema->getRegionSchema()->getRegion($region != ''? $region: RegionModelSchema::DEFAULT_PATH);
			...
			$tok = null; $tok_reg = null;
			foreach ($par->inregion as $reg) {
				if($tok != null) $p->ensureRegisteredInRegion($tok_reg, (string)$tok['date_start'], (string)$tok['date_end'], true);
				$tok_reg = $this->global_schema->getRegionSchema()->getRegion((string)$reg['region']);
				$tok = $reg;
			}
			
			if($tok != null) $p->ensureRegisteredInRegion($tok_reg, (string)$tok['date_start'], (string)$tok['date_end']); //commit
		}
