...
//=============- Message types/classes -==============
	...
	protected $typemask = self::ALL_MASK;
	...
	protected static function configureLogger(JLogger $logger) {
							...
							if($cl != '') {
								...
								if(class_exists($cl)) {
									...
									foreach ($hndl->param as $p) {
										$pnm = trim((string)$p['name']);
										if($pnm != '') $params[$pnm] = (string)$p['value'];
		...
		if(isset(self::$config['$'])) { //default config
			...
			if(isset($cf['handlers'])) foreach($cf['handlers'] as $hndl) {
				$logger->addLogHandler($hndl);
			}
		}
...
* @author Sardar Yumatov (ja.doma@gmail.com)
*/
class FileJLoggerHandler implements JLoggerHandler {
    ...
    public function __construct($params = null) {
        if(!isset($params['file'])) {
			trigger_error("FileJLoggerHandler: parameter 'file' is required, logging to file is disbled!", E_USER_WARNING);
        } else {
        	$file = $params['file'];
        	//[FIXME: absolute path check for POSIX and Windows paths only!]
        	if(!preg_match('#^\s*(?:/|(?:[a-zA-Z]:))#', $file)) {
        		if(!defined('JLOGGER_FILE_BASE')) {
        			trigger_error("Can't resolve relative log file path, JLOGGER_FILE_BASE constant is not defined!", E_USER_WARNING);
        			return;
				}

        		$file = JLOGGER_FILE_BASE.'/'.$file;
			}
        	$this->file = fopen($file, 'a');
        	if(!is_resource($this->file)) trigger_error("FileJLoggerHandler: Can't open file for appending: {$params['file']}", E_USER_WARNING);
		}
    }

    /**
     * Flushes stream, closes file.
     * @return void
     */
