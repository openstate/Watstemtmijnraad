<?php
require_once('functions.include.php'); //importing _id()
...
class Record {
	...
	*/
	...
	/**
	 * Special fields that don't throw exceptions if they are not set.
	 * The object may declare ghost variables assuming it provides the values not in each query
	 * (result of a selective join). Actually it is a dirty hack that is worsening the readabllity, but
	 * is necessary.
	 *
	 * Warning: ghost variables are *NOT* needed if a ORM class exists for all possible object variations.
	 * This is not the case, for example for Category, which can be a plain category or bound to a level (has
	 * ghost field 'level_description').
	 *
	 * @var array (string name => default_value)
	 */
	...
	*/
	...
	/*
		...
		Parameters:
		$data - The array to load the record from. The record's <$id> will be read from the key named 'id'.
	*/
	...
	*/
	public function load($id) {
		$parts = $this->getParts();
		$row = $this->db->query('SELECT t.*'.$parts['extra'].' FROM '.$parts['tables'].' WHERE t.id=%i', $id)->fetchRow();
		if ($row) {
	...
	*/
	public function getList($join = '', $where = '', $order = '', $limit = '', $exghoscols = '') {
		$parts = $this->getParts();

		$rows = $this->db->query('SELECT t.*'.$parts['extra'].($exghoscols? ", {$exghoscols}": '').' FROM '.$parts['tables'].' '.$join.' '.$where.' '.$order.' '.$limit)->fetchAllRows();
	...
	*/
	public function hasProperty($name) {
		return array_key_exists($name, $this->data) || array_key_exists($name, $this->ghostCols);
	}
	...
	public function __set($name, $value) {
		if ($this->hasProperty($name)) {
			...
			if (in_array($name, $this->nonwritable) || isset($this->extraCols[$name]) || array_key_exists($name, $this->ghostCols))
	...
	public function __get($name) {
		...
		} else if(array_key_exists($name, $this->ghostCols)) {
			return $this->ghostCols[$name];
		} else {
			throw new NoSuchPropertyException('Undefined property: '.get_class($this).'::$'.$name);
		}
	}
