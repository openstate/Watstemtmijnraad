...
class Dispatcher {
	...
	*/
	...
	public static function notFound() {
		...
		echo 'Requested document is not found.';
	...
	*/
	...
	public static function sessionUser() {
		return @$_SESSION['user_'.Dispatcher::inst()->activeSite['publicdir']];
	}
	...
	*/
	protected function initSmarty() {
		if (!$this->smarty) {
            ...
            if ($this->preview)
                output_add_rewrite_var('preview', $this->preview);
			...
			if (isset($_GET['extern'])) {
				...
				$this->smarty->setBaseTemplate($_SERVER['DOCUMENT_ROOT'].'/../templates/large_iframe.html');
				$this->smarty->assign('iframe', $this->iframe);
				//remove if mod_rewrite prefix is used
				output_add_rewrite_var('iframe', $this->iframe);
			} else {
			...
			if (file_exists($_SERVER['DOCUMENT_ROOT'].'/images/watstemtmijnraad/skins/custom/'.$subdomain.'/skin.css'))
				$this->smarty->assign('skin', $subdomain);
		}
	}
	...
	*/
	protected function setHost($hostInfo) {
		...
		if($this->subdomain == ''){
			$this->header('http://www.'.$this->domain.'.'.$this->tld.$_SERVER['SCRIPT_URL'].(@$_GET ? '?'.http_build_query($_GET) : ''));
		}
	}
	...
	*/
	public function dispatch() {
		...
		//	1 - limit to gemeente
		//	2 - limit to party
		$this->iframe = isset($_REQUEST['iframe']) && trim($_REQUEST['iframe'])? intval(trim($_REQUEST['iframe'])): 0;
        
        //preview mode
        $this->preview = isset($_REQUEST['preview']) && trim($_REQUEST['preview'])? intval(trim($_REQUEST['preview'])): 0;
		...
		$r = new Region();
		$this->region = $r->loadBySubdomain($this->subdomain);
		$url = $_SERVER['SCRIPT_URL'];
		if ($url == '/' && $this->region)
			$url = '/regions/region/'.$this->region->id;
		...
		if ($this->isFrontoffice()) {
			...
			$p = new Page();
			$pages = $p->getVisiblePages(2); //Region with ID 2 == Nederland
			if(isset($this->region)){
				foreach($p->getVisiblePages($this->region->id) as $p){
					$pages[] = $p;
				}
			}
			$this->smarty->assign('footer_pages', $pages);

			//default header for regional pages, may be overwritten later: case8875
			if($this->region) {
				$this->smarty->assign('header', $this->region->name);
			}
		}
		...
		if ($this->pageInfo && file_exists($this->pageInfo['file'])) {
			try {
                ...
                // Enable or disable debugging output in your settings.private.php
				if(DEBUG) {
					$log = DBs::inst(DBs::SYSTEM)->getLastQuery(-1);
