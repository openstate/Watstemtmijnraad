...
class dbpagesHandler {
	...
	public $urlMap = array(
		'!^/([a-zA-Z0-9]*)/?$!' => 'handler',  //[FIXME: dangerous pattern, can hide special pages like /search]
		'!^/page/([a-zA-Z0-9]*)/?$!' => 'handler',
		'!^/page/([0-9]+)/([a-zA-Z0-9]+)/?$!' => 'handlerRegion',
	...
	public function handlerRegion($match, array $pageSets) {
		require_once('Page.class.php');
		$p = new Page();

		if (null != $this->page = $p->loadByUrl($match[2], intval($match[1]))) {
			return array(
				'file'  => dirname(__FILE__).'/pages/view/php/indexPage.class.php',
				'class' => 'indexPage',
				'get'   => array('id' => $match[2], 'region' => $match[1])
			);
		} else
			return false;
	}
